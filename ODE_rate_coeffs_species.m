...
% Ocean reservior is from MIT GCM (Yanxu Zhang); others are from Amos et al., (2014)
%--------------------------------------------------------------------------
% Atmosphere (Mg)
...
% Ocean flux is from MIT GCM (Yanxu Zhang); others are from Amos et al. (2014)
%-------------------------------------------------------------------------
% Atmosphere (Mg/year)
Fatm_Hg0_Hg2  =   15200;        % Hg0 oxidation to Hg2 by Br, update based on Horowitz et al. (2017, ACP)
Fatm_Hg0_ocs  =   1700;            % gross uptake of Hg0 from the atmopshere (ref: HMA, v9-01-02, 2007-2009 avg),update based on Horowitz et al. (2017, ACP)
Fatm_Hg0_tf   =   1200*2;        % Hg0 only deposition to fast soil, increased based on Hg isotope work
Fatm_Hg2_Hg0  =   10500;        % Hg2 photo-reduction to Hg0, update based on Horowitz et al. (2017, ACP) 
Fatm_Hg2_ocs  =   4600;       % Hg2 deposition to the ocean, update based on Horowitz et al. (2017, ACP) 

Fatm_Hg0_soil   =   Fatm_Hg0_tf;      
Fatm_Hg2_soil   =   1000; % update based on Horowitz et al. (2017, ACP) 
...
E_bb        =   100*2.2;                 % biomass burning emissions,update based on Horowitz et al. (2017, ACP)
...
f_soilEF      =   600/160;         % Soil emission are increased by x times

% Fast terrestrial
Ftf_Hg0_atm_res  =   46*f_soilEF;           % evasion due to respiration of organic carbon, decreased by 10x according to Amos et al.(2014)
Ftf_Hg0_atm_pr   =   47*f_soilEF;           % photoreduction, decreased by 10x according to Amos et al., (2014)
Ftf_Hg0_atm_r    =   37.5*f_soilEF;         % re-emission of deposited Hg0 (Smith-Downey et al. 2010), decreased by 10x according to Amos et al.(2014)
Ftf_ts           =   325;          % exchange among soil pools, fast to slow pool
Ftf_ta           =   9;            % exchange among soil pools, fast to armored pool 
Ftf_bb         =  (E_bb*fveg+(E_bb*fsoil*fCfast)); % pre-1450 biomass burning

% Slow terrestrial
Fts_Hg0_atm_res  =    25*f_soilEF;         % evasion due to respiration of organic carbon,decreased by 10x according to Amos et al.(2014)
Fts_tf           =    205;          % exchange among soil pools, slow to fast pool
Fts_ta           =    0.5;          % exchange among soil pools, slow to armored pool 
Fts_bb          =    (E_bb*fsoil*fCslow);         % pre-1450 biomass burning

% Armored terrestrial
Fta_Hg0_atm_res  =     2.5*f_soilEF;          % evasion due to respiration of organic carbon, decreased by 10x according to Amos et al.(2014)
Fta_tf           =     15;           % exchange among soil pools, armored to fast pool 
Fta_m            =     0;            % exchange from armored pool to mineral pool (18 Dec 2011, hma)
...
% Ocean fluxes from MIT GCM (Mmol/yr)
Fatm_Hg0_ocs_MIT  =  13.6;           % gross uptake of Hg0 from the atmopshere, not used in the model
Fatm_Hg2_ocs_MIT  =  19.3;           % Hg2 deposition to the ocean, not used in the model
Focs_Hg0_atm_MIT  =  29.6;           % gross ocean evasion to the atmosphere, not used in the model
Focs_Hg0_atm     =   4700*1;         % gross ocean evasion to the atmosphere (Soerensen et al., 2010)
                                     % corrected based on net from Soerensen et al. (2010) and gross from Holmes et al. (2010)
% surface ocean 
...
Focd_Hg0_oci_v_MIT   = 1.68;     % advection: upward water transport of Hg0 from deep to intermediate ocean
Focd_Hg2_oci_v_MIT   = 6.94;     % advection: upward water transport of Hg2 from deep to intermediate ocean
Focd_HgP_oci_v_MIT   = 0.0048;   % advection: upward water transport of HgP from deep to intermediate ocean
Focd_Hg0_oci_d_MIT   = 2.92e-2;  % diffusion: upward transport of Hg0 from deep to intermediate ocean
...
Kta_bb           =  Fta_bb/Rta;             % pre-1450 biomass burning
...
Ksoil_bb           =  Fsoil_bb/Rsoil;             % pre-1450 biomass burning
%--------------------------------------------------------------------------
...
Kocs_HgP_Hg2_dm =  Focs_HgP_Hg2_rm_MIT                       /  Rocs_HgP_MIT;     % demineralization of HgP to Hg2
...
Koci_HgP_Hg2_dm = Foci_HgP_Hg2_rm_MIT                       /  Roci_HgP_MIT;   % demineralization of HgP to Hg2
...
%--------------------------------------------------------------------------
IHgD_pristine      =    76.9590;   %  Hg2 budget of reiver discharge (Mg/yr), reaching ocean margin, background value, Amos et al.(2014)
...
f_HgPexport        =    0.30;      % global fraction of riverine HgP reaching the open oceans (Table 2, Amos et al., 2014)
